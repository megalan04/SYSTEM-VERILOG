...
(.clk(clk),.rst_n(rst_n),.ena(ena),.din_from_bit8(dataio),.cmd_ack_from_bit8(cmd_ack_internal),.din_byte(din_byte),.cmd_in(cmd_in),.din(din),.read_byte(read_byte),.cmd_ack(cmd_ack), .rx_ack(rx_ack),.read_valid(read_valid),.cmd_out(cmd_from_byte));
endmodule
