...
`include "apb_i2c_master.sv"
`include "apb_i2c_slave.sv"
`include "i2c_master.sv"
`include "i2c_clk_gen.sv"
`include "byte_ctrl.sv"
`include "bit_ctrl.sv"
...
module apb_i2c_top #(parameter ADDR_WIDTH=16,
               ...
               apb_i2c_slave #(.CLK_CNT(CLK_CNT)) inst_slave
                (//board inputs
 ...
 .psel(psel),.penable(penable),.pwrite(pwrite),.paddr(paddr),.pwdata(pwdata),
                ...
                (
                  .clk(pclk),.rst_n(presetn),.clk_cnt(clk_cnt),.ena(ena), .din_byte(din_byte),.cmd_in(cmd_in),.cmd_ack(cmd_ack),.rx_ack(rx_ack), .read_byte(read_byte),.read_valid(read_valid),.busy(busy),.al(al), .sda(sda), .scl(scl));
endmodule
